...
PHYSICAL_SECTIONS = {'forces', 'energy', 'light', 'sound', 'heat',
                     'electric', 'magnets', 'matter'}
...
# History's one extra rule, and it is the whole reason the section works.
#
# Jacob chose the Bible's timeline as the spine. That does NOT license printing
# its years as bare fact while marking everyone else's as reasoning — that is
# the exact failure this app exists to correct, pointed the other way. Any card
# that states a YEAR is making a claim somebody had to arrive at: by adding up
# genealogies, by stacking king-lists, by carbon. So a year may never be `found`.
#
# `found` is for measurement. You can measure a pyramid. You cannot measure 4004 BC.
YEAR_CLAIM = re.compile(r'\b\d{3,4}\s*(BC|B\.C\.|BCE)\b'
                        r'|\bAD\s*\d{3,4}\b'
                        r'|\b\d{3,4}\s*(AD|CE)\b', re.I)

def check_history(e, sections, label):
    p = check_flat(e, sections, label)
    blob = ' '.join([e.get('text', ''), e.get('more') or ''])
    for m in YEAR_CLAIM.finditer(blob):
        if e.get('kind') == 'found':
            p.append(f'{e.get("id", "?")}: EDITORIAL — ancient year {m.group(0)!r} '
                     f'marked `found`; a year is worked out, never measured')
            break
    return p
...
def write_js(path, const, rows, header):
    with open(path, 'w', encoding='utf-8') as f:
        ...
        # `const X` at script top level is a LEXICAL binding, not a property of
        # window — the same trap that left window.Sync undefined and cloud sync
        # dead. TOPIC_SETS resolves its data by name, so the name has to really
        # be there. globalThis rather than window so the node-based corpus
        # walker in gen_audio.py can evaluate this file too.
...
def main():
    ...
    # One loop, because five near-identical blocks is five places for a rule to
    # be applied four times.
    NEW = [
        ('ancient',  'ANCIENT',  ANCIENT_SECTIONS,  'Ancient history', True),
        ('america',  'AMERICA',  AMERICA_SECTIONS,  'American history', True),
    ...
    for key, const, secs, human, is_history in NEW:
        rows = []
        for e in load(f'.work/entries-{key}-*.json'):
            pr = check_history(e, secs, key) if is_history else check_flat(e, secs, key)
            problems += pr
            if not any('EDITORIAL' in x or 'empty text' in x for x in pr):
                rows.append(e)
        order = list(secs)
        rows.sort(key=lambda e: (order.index(e['section'])
                                 if e.get('section') in order else 99, e.get('id', '')))
        dup = [e['id'] for e in rows]
        dup = sorted({i for i in dup if dup.count(i) > 1})
        if dup:
            problems.append(f'duplicate {key} ids: {", ".join(dup)}')
        n = write_js(os.path.join(ROOT, 'js', f'{key}.js'), const, rows,
                     f'// Wonder Lab {human.lower()} data. Generated by tools/build_extras.py\n'
                     + ('// Years are never `found` — see check_history and .work/BRIEF-ancient.md.\n'
                        if is_history else '')
                     + f'// {len(rows)} facts across '
                       f'{len({e["section"] for e in rows}) if rows else 0} sections.\n')
        print(f'wrote {len(rows)} {human.lower()} facts -> js/{key}.js ({n//1024} KB)')
